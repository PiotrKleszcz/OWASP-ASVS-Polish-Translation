...
| **3.3.4** | Zweryfikuj, że jeśli wartość ciasteczka nie ma być dostępna dla skryptów po stronie klienta (jak token sesji), ciasteczko musi mieć ustawiony atrybut 'HttpOnly', a ta sama wartość (np. token sesji) może być przekazywana klientowi wyłącznie poprzez pole nagłówka 'Set-Cookie'. | 2 |
